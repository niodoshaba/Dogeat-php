...
$.ajax({
    url: "/front/index.php?action=CartRender&controller=CartControllers",
    type: "GET",
    dataType: "text",
    cache: false,
            ...
            for(i=0;i<render_data.length;i++){
                ...
                $(".del_in_check").each(function(){
                    $(this).on("click",function(){
                        ...
                        $.ajax({
                            url: "/front/index.php?action=DeleteCart&controller=CartControllers",   //後端的URL
                            type: "POST",
                            dataType: "json",
                            cache: false,
...
$("#final_check").click(function(){
    if($(".each_product").length == 0){
        ...
        $.ajax({
        url: "/front/index.php?action=CreateOrder&controller=OrderControllers",   //後端的URL
        type: "POST",
        dataType: "text",
        data:order_data,
        cache: false,
...
function CleanCart(res){
    $.ajax({
    url: "/front/index.php?action=CleanCart&controller=CartControllers",   //後端的URL
    type: "GET",
    dataType: "text",
    cache: false,
    success: function(){
...
function EarnPoint(){
    $.ajax({
    url: "/front/index.php?action=EarnPoint&controller=MemberControllers",   //後端的URL
    type: "GET",
    dataType: "text",
    cache: false,
    data:{
...
function UsePoint(){
    $.ajax({
    url: "/front/index.php?action=UsePoint&controller=MemberControllers",   //後端的URL
    type: "GET",
    dataType: "text",
    cache: false,
    data:{
...
$("#ecpay").click(function(){
    if($(".each_product").length == 0){
        ...
        console.log("sdf");
        $.ajax({
        url: "/front/index.php?action=Ecpay&controller=OrderControllers",   //後端的URL
        type: "POST",
        dataType: "text",
...
$("#carry_on").click(function (e) {
    if(carry_on_hit_count == 1){
        ...
        location.href = "/front/index.php?action=product&controller=Home";
    }
});
